import { Role, TextBasedChannel, TextChannel } from "discord.js";
...
export default new Command({
	...
	run: async ({ client, message, lang, utils }) => {
		...
		async function askQuestions(i: number, askQuestion: boolean) {
			...
			const res = await utils.waitForResponse(message.author.id, message.channel as TextChannel);
		...
		async function finishAnnouncement() {
			...
			ch.send({
				content: tagRoles.length ? tagRoles.join(" ") : toTag,
				embeds: [
					client.embed.create({
						...
						Footer: {
							text: utils.replaceText(Announce.Announcement.Footer, "USER", message.author.tag),
						},
